...
def is_union_type_node(obj: object) -> TypeIs[UnionType]:
...
class AttrsFieldDef(FieldDef):
    ...
    eq: bool = True
    alias: str | None = None
    converter: bool = False  # True if has converter
    validator: bool = False  # True if has validator


@dataclass(slots=True)
class AttrsType(StructuredType):
    """An attrs class with typed fields."""

    cls: type
    fields: tuple[AttrsFieldDef, ...]
    frozen: bool = False
    slots: bool = True  # Default True in modern attrs
...
def is_attrs_type_node(obj: object) -> TypeIs[AttrsType]:
    """Return whether the argument is an AttrsType instance."""
...
@dataclass(slots=True)
class PydanticModelType(StructuredType):
    """A Pydantic BaseModel with typed fields."""

    cls: type
    fields: tuple[PydanticFieldDef, ...]
    frozen: bool = False
    strict: bool = False
    extra: str = "ignore"  # "ignore", "allow", "forbid"
    _children: tuple[TypeNode, ...] = field(
        init=False, repr=False, compare=False, hash=False
    )
...
def is_pydantic_model_type_node(obj: object) -> TypeIs[PydanticModelType]:
    """Return whether the argument is a PydanticModelType instance."""
    return isinstance(obj, PydanticModelType)
